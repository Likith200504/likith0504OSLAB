...
void fcfs(int n, int queue_type)
{
    ...
    for (i = 0; i < n; i++)
    {
        if (queue[i] == queue_type && remaining_time[i] > 0)
        {
            ...
            printf("P%d (Queue %d) finished at time %d\n", pid[i], queue_type, time);
        }
    }
}
...
int main()
{
    ...
    printf("\nExecuting System Processes (Queue 1 - Higher Priority)\n");
    fcfs(n, 1);

    printf("\nExecuting User Processes (Queue 2 - Lower Priority)\n");
    fcfs(n, 2);
